...
def _ensure_manifest() -> None:
    ...
    if secret_manifest.is_file():
        ...
        _log.info("manifest loaded from Render secret file /etc/secrets/services.yaml")
        return
    _log.error(
...
for _n, _s in SERVICES.items():
    ...
    for _p in _api.get("public_paths") or []:
        ...
        PUBLIC_PREFIXES.append(
            f"{_mount}{_p}" if _p.startswith("/") else f"{_mount}/{_p}"
        )
...
def _normalize_ssh_key_file() -> None:
    ...
    # Best-effort on Windows/containers where chmod is a no-op or denied: the
    # key is already written, so a failed permission tightening must not abort
    # the boot (ssh only refuses to use it on POSIX, where chmod does work).
...
def _check_deps(name: str, deps: list | None) -> None:
    ...
    if missing:
        _log.warning(
            ...
            "(add them to pyproject.toml, run `uv lock` and redeploy)",
            name,
            ", ".join(missing),
        )
...
def _mount_services() -> None:
    for name, svc in SERVICES.items():
        ...
        if (
            mount
            and entry.get("ok")
            and getattr(entry["module"], "app", None) is not None
        ):
...
async def _exec_action(name: str, action_spec: dict) -> dict:
    ...
    try:
        ...
        if action_spec.get("sync") and not asyncio.iscoroutinefunction(target):
            raw = await asyncio.wait_for(run_in_threadpool(target, *args), timeout)
        else:
...
# -- Fetch + smart reload (per-service, no full process restart) ------------

TICK_LOCK = asyncio.Lock()  # one tick/fetch at a time (no double runs)


def _reload_service(name: str) -> bool:
    """Hot-reload a single service module: pop the old module from sys.modules,
    re-import the (changed) file, re-run setup, re-mount the sub-app.

    Returns True on success, False on failure (caller falls back to process
    restart via _graceful_exit if RESTART_ON_CHANGE is on)."""
    ...
    path = SERVICES_DIR / dest
    if not path.is_file():
        _log.error("hot-reload %s: file %s missing", name, dest)
        return False
    mod_name = f"svc_{name}"

    # 1) unmount the old sub-app (find and remove its Mount route)
    mount_path = (svc.get("api") or {}).get("mount")
    if mount_path:
        app.router.routes = [
            r
    ...
    # 2) pop old module so exec_module loads fresh code
    sys.modules.pop(mod_name, None)
    REGISTRY.pop(name, None)

    # 3) re-validate env + re-import + re-setup + re-mount
    try:
        env_err = _validate_env(name, svc.get("env") or {})
        if env_err:
            REGISTRY[name] = {"ok": False, "error": env_err}
            _log.error("hot-reload %s: %s", name, env_err)
            return False
        module = _import_module(mod_name, path)
        for attr in svc.get("setup_dirs") or []:
            obj = module
        ...
        for fn in svc.get("setup") or []:
            ...
            if callable(call):
                call()
        REGISTRY[name] = {"ok": True, "error": None, "module": module}
        if mount_path and getattr(module, "app", None) is not None:
            app.mount(mount_path, module.app)
        _log.info("hot-reload %s: success (code refreshed in place)", name)
        return True
    except Exception as exc:
        REGISTRY[name] = {"ok": False, "error": str(exc)[:300]}
        _log.error("hot-reload %s FAILED: %s (falling back to full restart)", name, exc)
        return False


def _map_changed_to_services(changed: list[str]) -> list[str]:
    """Map changed file dests (e.g. 'committer/app.py') back to service names."""
    result = []
    for name, svc in SERVICES.items():
        entry = svc.get("entry", "")
        # `changed` holds dest paths (sources.fetch_all returns spec["name"],
        # i.e. the file's "dest"), so the comparison must be against the dest
        # strings. Iterating the file dicts themselves never matched, so a
        # changed source file never triggered its service's hot-reload.
        files = (svc.get("source") or {}).get("files") or []
        dests = [f.get("dest") for f in files]
        if entry in changed or any(d in changed for d in dests):
            result.append(name)
    return result
...
async def _do_fetch(state: dict) -> dict:
    ...
    If something changed -> hot-reload per service in-place; full process
    restart only as fallback when a hot-reload fails."""
    ...
    if fetch_info["changed"]:
        changed_services = _map_changed_to_services(fetch_info["changed"])
        reload_report = {}
        any_failed = False
        for name in changed_services:
            ok = _reload_service(name)
            reload_report[name] = "ok" if ok else "failed"
            if not ok:
                any_failed = True
        fetch_info["reloaded"] = reload_report
        # only fall back to full process restart if a hot-reload failed
        if any_failed and RESTART_ON_CHANGE:
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request, call_next):
        ...
        if (
            ...
            or path.startswith(
                "/credit/api/"
            )  # browser dashboard: original service had zero auth
        ):
            return await call_next(request)
...
async def tick() -> JSONResponse:
    ...
    async with TICK_LOCK:
        ...
        for name in SERVICES:
            ...
            if r.get("skipped"):
                ...
                summary_lines.append(
                    f"  {display}: OK ({ms}ms){f' — {hint}' if hint else ''}"
                )
                ok_count += 1
            else:
        ...
        summary_lines.insert(
            0, f"Services: {ok_count} ok, {err_count} error, {skip_count} skipped"
        )
...
async def service_action(name: str, action: str) -> JSONResponse:
    ...
    if name not in SERVICES_ENABLED:
        return JSONResponse({"skipped": True, "reason": "disabled (SERVICES_ENABLED)"})
    entry = REGISTRY.get(name, {})
    if not entry.get("ok"):
        return JSONResponse(
    ...
    return JSONResponse({"service": name, "action": action, "result": result})
...
async def status() -> dict:
    ...
    for name, svc in SERVICES.items():
        ...
        if st_spec and entry.get("ok"):
            ...
            info["status"] = (
                res.get("data") if res.get("ok") else {"error": res.get("error")}
            )
        services[name] = info
    return {
        "ts": _now().isoformat(),
